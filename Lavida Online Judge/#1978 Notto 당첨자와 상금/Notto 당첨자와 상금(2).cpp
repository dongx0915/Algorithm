...
int main() {
	...
	while (test--) {
		...
		for (i = 1; i <= 4; i++) {
			if (cnt_rank[i] == 0) dev_money[i] = 0;
			else {
				switch (i) {
				case 1: dev_money[i] = (money / 2) / cnt_rank[1]; break;
				case 2: dev_money[i] = (money / 4) / cnt_rank[2]; break;
				case 3: dev_money[i] = ((money / 100) * 15) /cnt_rank[3]; break;
				case 4: dev_money[i] = ((money / 100) * 5) / cnt_rank[4]; break;
				}
			}
		}
